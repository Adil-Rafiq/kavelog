...
### Push reminders (on by default, push-only)
- Web Push nudges to keep the personal log complete. **Push-only** — reminders never create in-app (`notify()`) rows, so the bell stays for real events. `users.remindersEnabled` is the account-level flag and **defaults to true**; the Account toggle turns it off/on. A reminder fires only when `remindersEnabled` is true AND the user has ≥1 subscription — so the flag alone delivers nothing until a device subscribes.
- **Getting the device subscribed is the real adoption step** (browsers only surface the permission prompt inside a click, so nothing is auto-prompted). Three click surfaces trigger `subscribeToPush()`: the Account toggle, the Account card's "Turn on here" button (shown when the flag is on but this browser isn't subscribed), and the dashboard `ReminderPrompt` banner (`reminders-prompt.tsx`) + the tour's "Allow notifications" finale. The banner uses permission *priming* — its "Enable" click is what fires the OS prompt — and only shows when reminders are on, push is supported, permission isn't already granted/denied, and this device has no subscription (snoozes a week on dismiss).
